...
- Added script to download clowder data to your local. [#414](https://github.com/clowder-framework/clowder/issues/414)
...
- _exact_ flag on collection and dataset API endpoints that accept a _title_ flag. This will use
...
- Only show spaces, collections and datasets that are shared with other users under 'explore' tab.
  In application.conf, this is set by the _showOnlySharedInExplore_ whose default value is false.
...
- Datasets can be copied with _Download Files_ and _View Dataset_ permissions instead of just the owner.
...
- Return _409 Conflict_ when submitting file for manual extraction and file is not marked as _PROCESSED_.
...
- Updated docker compose repositories from ncsa/_ to clowder/_.
...
- _(Experimental)_ Support for geostreams datapoints with parameters values organized by type.
