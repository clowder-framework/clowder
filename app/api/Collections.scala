          ...
          (request.body \ "description").asOpt[String].map { description =>
              (request.body \ "space").asOpt[String].map { space => 
                ...
                user match {
                  case Some(identity) => {
                    if (space == "default") {
                      c = Collection(name = name, description = description, created = new Date(), author = identity)
                    }
                    else {
                      c = Collection(name = name, description = description, created = new Date(), author = identity, spaces = List(UUID(space)))
                    }
