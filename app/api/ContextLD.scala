...
class ContextLD @Inject() (
  ...
  def getContextById(id: models.UUID) = SecuredAction(parse.anyContent, authorization = WithPermission(Permission.ShowContext)) {
    request =>
  ...
  def getContextByName(contextName: String) = SecuredAction(parse.anyContent, authorization = WithPermission(Permission.Public)) {
    request =>
  ...
  def addContext() = SecuredAction(authorization = WithPermission(Permission.CreateContext)) {
    implicit request =>
      request.user match {
        case Some(user) => {
  ...
  def removeById(id: models.UUID) = SecuredAction(parse.anyContent, authorization = WithPermission(Permission.DeleteContext)) {
    implicit request =>
      request.user match {
        case Some(user) => {
