...
class Datasets @Inject()(
   ...
   * Dataset.
   */
  def dataset(id: UUID, spaceTraced:Option[String]) = PermissionAction(Permission.ViewDataset, Some(ResourceRef(ResourceRef.dataset, id))) { implicit request =>
      ...
      datasets.get(id) match {
        case Some(dataset) => {
          ...
          decodedCollectionsOutside.toList, decodedCollectionsInside.toList, filesOutside, isRDFExportEnabled, Some(decodedSpaces), filesTags, otherSpaces, spaceTraced))
