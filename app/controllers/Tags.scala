...
import services.{CollectionService, DatasetService, FileService, SectionService, SearchService}
import util.Parsers
...
class Tags @Inject()(collections: CollectionService,
  ...
  def createTagList(user: Option[User]) = {
    ...
    if (searches.isEnabled()) {
      ...
      Logger.error("Search service could not be reached for tag search; using Mongo instead")
