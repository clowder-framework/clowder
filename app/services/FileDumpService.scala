...
class FileDumpService(application: Application) extends Plugin {
  ...
  def dump(fileDump: DumpOfFile) = {
    ...
    fileDumpDir match {
      case Some(dumpDir) => {
        val fileSep = System.getProperty("file.separator")
        val fileDumpingDir = dumpDir + fileDump.fileId.charAt(fileDump.fileId.length() - 3) + fileSep +
          fileDump.fileId.charAt(fileDump.fileId.length() - 2) + fileDump.fileId.charAt(fileDump.fileId.length() - 1) +
          fileSep + fileDump.fileId + fileSep + fileDump.fileName
