...
class MongoDBCollectionService @Inject() (
  ...
  private def count(date: Option[String], nextPage: Boolean, title: Option[String], space: Option[String], permissions: Set[Permission], user: Option[User], showAll: Boolean, owner: Option[User]): Long = {
    ...
    val (filter, _) = {
      if (showAll){
        filteredQuery(date, nextPage, title, space, Set[Permission](Permission.AddResourceToCollection), user, showAll, owner)
      } else {
        filteredQuery(date, nextPage, title, space, Set[Permission](Permission.ViewCollection), user, showAll, owner)
      }
    }
