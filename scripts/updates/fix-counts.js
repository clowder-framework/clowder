db.app.configuration.update({"key" : "mongodb.updates"}, { $addToSet: {"value": "update-space-status"}})
	...
	db.datasets.find({"spaces": s._id}).forEach(function(d) {
		d.files.forEach(function(fid) {
			db.uploads.find({"_id": fid}).forEach(function(f) {
				spaceBytes += f.length.toNumber();
				filecount += 1;
			});
		});
		db.folders.find({"parentDatasetId": d._id}).forEach(function(folder) {
			folder.files.forEach(function(fid) {
				db.uploads.find({"_id": fid}).forEach(function(f) {
					spaceBytes += f.length.toNumber();
					filecount += 1;
				});
